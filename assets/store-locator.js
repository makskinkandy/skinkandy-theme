...
function createMapContent(place) {
  return `<h6>${place.name}</h6> <p>${place.vicinity}</p> ${getOpenStatus(place)} <a href="https://www.google.com/maps/search/?api=1&query=${place.geometry.location.lat()},${place.geometry.location.lng()}" class="direction>Get Directions</a>`;
}
