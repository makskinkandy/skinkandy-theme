...
(function(){
  ...
  theme.Variants = (function() {
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _updateMasterSelect: function(variant) {
        if(!this.container.querySelector(this.originalSelectorId)) return;
  ...
  theme.CartForm = (function() {
    ...
    function CartForm(form) {
      ...
      this.quickAddPopupEnabled = form.dataset.quickaddPopup;
      
      if(this.quickAddPopupEnabled == "true") {
        const quickAddVariantId = form.dataset.quickaddProduct;
        ...
        .then(data => {
          data.items.forEach(item => {
            ...
            if(variantId == quickAddVariantId) {
              this.quickAddPopupEnabled = "false";
            }
          })
        });
      }
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      onSubmit: function(evt) {
        ...
        } else if(this.quickAddPopupEnabled == "true") {
          evt.preventDefault();
          this.quickAddPopup();
        }
      },
      ...
      quickAddPopup: function() {
        this.quickAddPopupModal = new theme.Modals('QuickAddPopup', 'quickadd-popup-modal');
        this.quickAddPopupModal.open();
        this.quickAddPopupEnabled = false;
        const goToCheckoutBtns = document.querySelectorAll('.js-goto-checkout');
        if(goToCheckoutBtns.length) {
      ...
      cartMarkup: function(html) {
        ...
        // Update subtotal
        const checkOutBtns = document.querySelectorAll(selectors.subTotal);
        checkOutBtns.forEach(el => {
          if (Shopify.country == "NZ") {
            el.innerHTML = "NZD " + theme.Currency.formatMoney(subtotal, theme.settings.moneyFormat);
          } else {
            el.innerHTML = theme.Currency.formatMoney(subtotal, theme.settings.moneyFormat);
          }
        });
      ...
      removeFromCart: function(e) {
        const ajaxRemove = e => {
          ...
          theme.cart.changeItem(key, 0)
            ...
            .catch(function(XMLHttpRequest){})
            .finally(self.classList.remove('btn--loading'));
        }

        const removeBtns = document.querySelectorAll('.js-remove-from-cart');
        removeBtns.forEach(btn => {
          btn.addEventListener('click', ajaxRemove, false);
        })
      },
  ...
  theme.preloadProductModal = function(handle, productId, btn) {
    let holder = document.getElementById('QuickShopHolder-' + handle);
    if (!holder) {
      return;
    }
    ...
    // Do no fetch if modal already has content that matches selected product handle
    if (holder.firstChild && holder.firstChild.dataset.productHandle === handle) {
    ...
    let url = theme.routes.home + '/products/' + handle + '?view=modal';
  ...
  theme.headerNav = (function() {
    ...
    function scrollHandler() {
      if (window.scrollY > 250) {
        ...
        document.body.setAttribute('data-header-sticky-transitioning', true);
        ...
        setTimeout(function() {
          ...
          document.body.setAttribute('data-header-sticky', true);
        }, 100);
      } else {
        if (!config.stickyActive) {
          return;
        ...
        document.body.removeAttribute('data-header-sticky-transitioning');
      ...
      output = `
        <div class="image-wrap">
          <img class="grid-product__image lazyload"
            ...
            data-sizes="auto"
            alt="${product.title}"
            style="aspect-ratio: ${product.aspect_ratio}; height: 100%; object-fit: contain">
        </div>`;
    } else {
  ...
  theme.Locations = (function() {
    ...
    function Locations(container) {
      this.location_containers = container.querySelectorAll(selectors.location_container);

      this.init();
    }

    Locations.prototype = Object.assign({}, Locations.prototype, {
      init: function () {
        this.location_containers.forEach( el => {
          let title_el = el.querySelector('.location-title'),
            locations_wrapper = el.querySelector('.locations');

          if (title_el.classList.contains('open')) {
            locations_wrapper.style.maxHeight = locations_wrapper.scrollHeight + "px";
          }
          title_el.addEventListener('click', function(e) {
            let _this = e.target;
            _this.classList.toggle('open');
            _this.classList.contains('open') ? locations_wrapper.style.maxHeight = locations_wrapper.scrollHeight + "px" : locations_wrapper.style.maxHeight = null;
          })
        })
      }
    });
  ...
  DOMready(function(){
    ...
    theme.sections.register('locations', theme.Locations);
